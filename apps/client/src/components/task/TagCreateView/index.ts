...
const mountComponent = (component: Component, props: Record<string | symbol, any>, eventListener: Record<string, any>) => {
    const app = createApp({
        setup() {
            return () => h(component, {
                ...props,
...
function TagDialog(options: TagDialogOptions = {}) {
    ...
    const { unmount } = mountComponent(TagCreateView, reactiveOptions, {
        'onCancel': () => {
            pRes('cancel')
        },
        'onClose': () => {
