...
import { SmartProjectNames } from '@/service/task'
import { useProjectMoreActions } from "@/composable/useProjectMoreActions.ts";

const { hideTaskItem, showTaskItem, whetherCanShow } = useProjectMoreActions()
...
<template>
  <ul p-8>
    <li
        ...
        :key="item.key"
        class="pl-4 pr-2 h-60px flex items-center"
    >
      ...
      <div w-200px>
        <NSelect
            ...
            :options="options"
            @update:value="handleChangeOption(item.key, $event)"
        />
      </div>
    </li>
  </ul>
